...
function Waitlist() {
    ...
    return (
        <div className="flex flex-col gap-8 items-center min-h-[85vh] relative overflow-hidden p-6 sm:p-10 -mx-4 sm:-mx-8 rounded-[2.5rem]">
            <div className="max-w-3xl w-full bg-[#121212]/70 backdrop-blur-xl border border-lavender/30 shadow-[0_0_40px_rgba(150,123,182,0.15)] rounded-[2rem] p-8 md:p-12 relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
                <div>
                    <h1 className="text-3xl font-extrabold text-lavender-light mb-2 flex items-center gap-3">
                        <Clock className="w-8 h-8 text-yellow-400" />
                        Your Waiting List
                    </h1>
                    <p className="text-gray-400">Track your queue position and seat allocation status.</p>
                </div>

                <div className="flex gap-2 p-1 bg-white/5 backdrop-blur-md border border-white/10 rounded-xl shrink-0">
                    {['waiting', 'confirmed', 'refunded', 'unsuccessful'].map((tab) => (
                        <button
                            key={tab}
            ...
            <div className="flex flex-col gap-4 max-w-3xl w-full relative z-10">
                {loading ? (
                    ...
                    <div className="bg-[#1a1a1a]/60 backdrop-blur-md rounded-2xl p-12 text-center border border-lavender/30 flex flex-col items-center w-full">
                        <AlertCircle className="w-12 h-12 text-lavender/40 mb-4" />
                        <p className="text-xl text-gray-400 font-bold">No {activeTab} requests found.</p>
                    </div>
                ) : filteredWaitlist.map((w, index) => (
                    <BookingCard
                        key={w.id}
                        type="waitlist"
                        {...w}
